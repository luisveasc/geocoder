...
func LoginFunc(c *gin.Context) (interface{}, error) {
	...
	if err := c.BindJSON(&loginVals); err != nil {
		return "", errors.New("Ingrese un usuario y una contraseña")
	}
...
func PayLoad(data interface{}) jwt.MapClaims {
	if v, ok := data.(model.User); ok {
		//usuario := model.User{Institution: v.Institution, Email: v.Email, Name: v.Name, ID: v.ID}
		usuario := model.User{Email: v.Email, Name: v.Name, ID: v.ID}
		claim := jwt.MapClaims{
			"user": usuario,
			"rol":  v.Rol,
		}
...
func LoadJWTAuth(login loginFunc) *jwt.GinJWTMiddleware {
	...
	var key string
	var set bool
	key, set = os.LookupEnv("GEOC_JWT_KEY")
	if !set {
